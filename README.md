...
In the vein of [infrastructure assrc="https://raw.githubusercontent.com/octodns/octodns/main/docs/logos/octodns-logo.png?" alt="octoDNS Logo" height=251 width=404>

## DNS as code - Tools for managing DNS across multiple providers

In the## Sources

Similar to providers, but can only serve to populate recordThe `class` key in the providers config section can be used to point to arbitrary classes in the python path so internal or 3rd party providers can easily be included with no coordination beyond getting them into PYTHONPATH, most likely installed into the virtualenv with octoDNS.into a zone, cannot be synced to.

| Source | Record Support | Dynamic | Notes |of [infrastructure as
