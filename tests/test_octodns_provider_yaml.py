    ...
    def test_provider(self):
        ...
        with TemporaryDirectory() as td:
            ...
            with open(yaml_file) as fh:
                ...
                self.assertTrue('values' in data['sub'])
                self.assertTrue('values' in data['txt'])
                self.assertTrue('values' in data['loc'])
                self.assertTrue('values' in data['urlfwd'])
                self.assertTrue('values' in data['sub.txt'])
                self.assertTrue('values' in data['subzone'])
                # these are stored as singular 'value'
                self.assertTrue('value' in data['_imap._tcp'])
                self.assertTrue('value' in data['_pop3._tcp'])
